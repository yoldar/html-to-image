...
function parseURLs(str) {
    ...
    return result.filter(function (url) { return !isDataUrl(url); });
}
function embed(cssString, resourceURL, baseURL, options) {
    ...
    return Promise.resolve(resolvedURL)
        .then(function (url) { return getBlobFromURL(url, options); })
        .then(function (data) { return toDataURL(data, getMimeType(resourceURL)); })
